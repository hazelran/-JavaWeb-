...
.bggray{ background:#f9f9f9}
    ...
    <script>
        function delUser(c) {
            if (confirm("您确定删除吗？")) {
                document.getElementById('del'+c).submit()
            }
        }
    </script>
</head>
  ...
  <tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="main-tab">
      ...
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        ...
        <td align="center" valign="middle" class="borderbottom"><a href="${pageContext.request.contextPath}/CommunityInfo?cname=${com.communityName}" target="mainFrame" onFocus="this.blur()" class="add">编辑</a>
            <span class="gray">&nbsp;|&nbsp;</span>
            <form id="del${status.index}" method="post" action="Update?ope=cdele">
                <input type="hidden" name="cname" value="${com.communityName}">
                <a target="mainFrame" onFocus="this.blur()" class="add" onclick="delUser(${status.index});">删除</a>
            </form>
        </td>
      </tr>
      </c:forEach>
